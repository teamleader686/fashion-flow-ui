  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
                      ...
                      <tr key={coupon.id} className="hover:bg-muted/30 transition-colors">
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end gap-1">
                            <Button variant="ghost" size="icon" onClick={() => handleViewUsage(coupon)}>
            ...
            <Card key={coupon.id}>
              <CardContent className="p-4">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <Badge variant={coupon.status === 'active' ? 'default' : 'secondary'} className="text-xs">
                        {coupon.status}
                      </Badge>
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    {coupon.max_discount && (
                      <div className="text-xs text-muted-foreground">max ₹{coupon.max_discount}</div>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                  <div>
                    ...
                    <span className="ml-1 font-medium">
                      {coupon.total_usage_count}{coupon.usage_limit && ` / ${coupon.usage_limit}`}
                    </span>
                  </div>
                  <div>
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => handleViewUsage(coupon)} className="flex-1">
                    <Eye className="w-4 h-4 mr-1" /> Usage
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => handleEdit(coupon)} className="flex-1">
                    <Edit className="w-4 h-4 mr-1" /> Edit
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => handleDelete(coupon.id)} className="text-destructive hover:text-destructive">
                    <Trash2 className="w-4 h-4" />
                  </Button>
                </div>
              </CardContent>
        ...
        <CouponUsageDialog
          open={usageOpen}
          onClose={() => { setUsageOpen(false); setSelectedCoupon(null); }}
