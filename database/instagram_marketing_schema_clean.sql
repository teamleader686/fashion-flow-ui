-- ============================================
-- INSTAGRAM MARKETING MODULE - CLEAN SCHEMA
-- ============================================
...
DROP TABLE IF EXISTS instagram_notifications CASCADE;
DROP TABLE IF EXISTS instagram_coin_logs CASCADE;
DROP TABLE IF EXISTS instagram_assignments CASCADE;
DROP TABLE IF EXISTS instagram_campaigns CASCADE;
DROP TABLE IF EXISTS instagram_users CASCADE;
DROP VIEW IF EXISTS instagram_analytics CASCADE;
DROP FUNCTION IF EXISTS create_expiry_reminders() CASCADE;
DROP FUNCTION IF EXISTS assign_instagram_coins(UUID, INTEGER, TEXT, UUID) CASCADE;
DROP FUNCTION IF EXISTS check_instagram_assignment_expiry() CASCADE;
DROP FUNCTION IF EXISTS update_instagram_user_coins() CASCADE;
DROP FUNCTION IF EXISTS update_instagram_updated_at() CASCADE;
...
CREATE TABLE instagram_assignments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE instagram_coin_logs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES instagram_users(id) ON DELETE CASCADE,
...
CREATE INDEX idx_instagram_users_status ON instagram_users(status);
CREATE INDEX idx_instagram_users_email ON instagram_users(email);
CREATE INDEX idx_instagram_campaigns_status ON instagram_campaigns(status);
CREATE INDEX idx_instagram_assignments_user ON instagram_assignments(user_id);
CREATE INDEX idx_instagram_assignments_campaign ON instagram_assignments(campaign_id);
CREATE INDEX idx_instagram_assignments_expiry ON instagram_assignments(expiry_date);
CREATE INDEX idx_instagram_coin_logs_user ON instagram_coin_logs(user_id);
CREATE INDEX idx_instagram_notifications_recipient ON instagram_notifications(recipient_id, recipient_type);
...
SELECT 
  ...
  EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'instagram_users') as created
UNION ALL
SELECT 'instagram_campaigns', EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'instagram_campaigns')
UNION ALL
SELECT 'instagram_assignments', EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'instagram_assignments')
UNION ALL
SELECT 'instagram_coin_logs', EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'instagram_coin_logs')
UNION ALL
SELECT 'instagram_notifications', EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'instagram_notifications');

-- ============================================
-- SUCCESS!
-- ============================================
-- If you see all tables with 'created = true', setup is complete!
-- You can now use the Instagram Marketing module.
