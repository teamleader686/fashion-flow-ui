-- ============================================
-- FINAL AFFILIATE INSTALL - COMPLETE IN ONE GO
-- ============================================
-- Yeh file sab kuch check karke install karegi
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'orders') THEN
        RAISE EXCEPTION 'Orders table does not exist. Please create main database schema first.';
    END IF;
END $$;

-- STEP 2: Add columns to orders table (if not exists)
DO $$ 
BEGIN
...
-- STEP 3: Add columns to products table (if not exists)
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'affiliate_commission_type') THEN
        EXECUTE 'ALTER TABLE products ADD COLUMN affiliate_commission_type TEXT';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'affiliate_commission_value') THEN
        EXECUTE 'ALTER TABLE products ADD COLUMN affiliate_commission_value DECIMAL(10,2)';
    END IF;
END $$;

-- STEP 4: Create affiliate tables
...
CREATE OR REPLACE FUNCTION process_commission_payment(p_affiliate_order_id UUID) RETURNS BOOLEAN AS $$
DECLARE v_affiliate_id UUID; v_commission_amount DECIMAL;
BEGIN
  SELECT affiliate_id, commission_amount INTO v_affiliate_id, v_commission_amount
  ...
  VALUES (v_affiliate_id, 'credit', v_commission_amount, 'commission', p_affiliate_order_id, 'Commission payment');
...
CREATE POLICY "Admin full access" ON affiliates FOR ALL TO authenticated
USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin'));

DROP POLICY IF EXISTS "Affiliates view own" ON affiliates;
CREATE POLICY "Affiliates view own" ON affiliates FOR SELECT TO authenticated
USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Anyone insert clicks" ON affiliate_clicks;
CREATE POLICY "Anyone insert clicks" ON affiliate_clicks FOR INSERT TO anon, authenticated
WITH CHECK (true);

DROP POLICY IF EXISTS "Admin all orders" ON affiliate_orders;
CREATE POLICY "Admin all orders" ON affiliate_orders FOR ALL TO authenticated
USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin'));

DROP POLICY IF EXISTS "Admin all commissions" ON affiliate_commissions;
CREATE POLICY "Admin all commissions" ON affiliate_commissions FOR ALL TO authenticated
USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin'));

DROP POLICY IF EXISTS "Affiliates view commissions" ON affiliate_commissions;
CREATE POLICY "Affiliates view commissions" ON affiliate_commissions FOR SELECT TO authenticated
...
CREATE POLICY "Admin all transactions" ON wallet_transactions FOR ALL TO authenticated
USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin'));

DROP POLICY IF EXISTS "Affiliates view transactions" ON wallet_transactions;
CREATE POLICY "Affiliates view transactions" ON wallet_transactions FOR SELECT TO authenticated
USING (affiliate_id IN (SELECT id FROM affiliates WHERE user_id = auth.uid()));

-- SUCCESS MESSAGE
SELECT '✅ AFFILIATE MARKETING SYSTEM INSTALLED!' as message;
SELECT 'Go to: /admin/affiliate-marketing' as next_step;
