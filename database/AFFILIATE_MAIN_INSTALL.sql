-- ============================================
-- MAIN AFFILIATE INSTALL - STEP 2
-- ============================================
-- Pehle RUN_THIS_FIRST_AFFILIATE.sql chalao
-- Uske baad yeh file chalao
-- ============================================
...
-- Indexes (sirf affiliate tables ke liye)
...
-- Note: orders table pe index Step 1 mein add ho gaya
...
BEGIN
  UPDATE affiliates SET total_orders = total_orders + 1, total_sales = total_sales + NEW.order_total,
    total_commission = total_commission + NEW.commission_amount, updated_at = NOW() WHERE id = NEW.affiliate_id;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  IF NEW.transaction_type = 'credit' THEN NEW.balance_after := current_balance + NEW.amount;
  ELSE NEW.balance_after := current_balance - NEW.amount; END IF;
...
DECLARE v_affiliate_id UUID; v_commission_amount DECIMAL; v_order_id UUID;
BEGIN
  ...
  VALUES (v_affiliate_id, 'credit', v_commission_amount, 'commission', p_affiliate_order_id, 'Commission for order');
...
CREATE POLICY "Affiliates can view own data" ON affiliates FOR SELECT TO authenticated USING (user_id = auth.uid());
...
-- Success
SELECT '✅ AFFILIATE SYSTEM INSTALLED SUCCESSFULLY!' as message;
SELECT 'Admin Panel: /admin/affiliate-marketing' as next_step;
